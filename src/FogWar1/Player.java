...
import FogWar1.FogWar1.GameMode;
...
public class Player {
    ...
    public void update(int delta) {
    	...
    	if(!isPlayer){
	        if(status != BotStatus.None){
	        	switch(status){
	        	case MoveRight:
	        		if(this.x>=arrivalX || this.map.isCollision(getFuturX(delta), getFuturY(delta))){
	        	...
	        	case MoveLeft:
	        		...
	        		break;
	        	case MoveUp:
	        		if(this.y>=arrivalY || this.map.isCollision(getFuturX(delta), getFuturY(delta))){
	        			status = BotStatus.None;
	    	        	arrivalY = Float.MAX_VALUE;
	    	        	dureePause = 0;
	    	        	this.setDx(0.0f);
	    	        	this.setDy(0.0f);
	        		}else{
	        	...
	        	case MoveDown:
	        		...
	        		break;
	        	case MoveUpRight:
	        		if((this.x>=arrivalX && this.y>=arrivalY) || this.map.isCollision(getFuturX(delta), getFuturY(delta))){
	        			status = BotStatus.None;
	    	        	arrivalX = Float.MAX_VALUE;
	    	        	arrivalY = Float.MAX_VALUE;
	    	        	dureePause = 0;
	...
	public void randomOrder(GameMode currentGameMode) {
		...
		if(Math.random()<0.4){
			...
			dureePause = 1 + (int)(Math.random() * ((1000 - 1) + 1));
		}else{
			float rdir = (float) Math.random();
			if(rdir<(1.0f/8.0f))
				status = BotStatus.MoveRight;
			else if(rdir<(2.0f/8.0f))
				status = BotStatus.MoveDownRight;
			else if(rdir<(3.0f/8.0f))
				status = BotStatus.MoveDown;
			else if(rdir<(4.0f/8.0f))
				status = BotStatus.MoveDownLeft;
			else if(rdir<(5.0f/8.0f))
				status = BotStatus.MoveLeft;
			else if(rdir<(6.0f/8.0f))
				status = BotStatus.MoveUpLeft;
			else if(rdir<(7.0f/8.0f))
				status = BotStatus.MoveUp;
			else if(rdir<=(7.0f/8.0f))
				status = BotStatus.MoveUpRight;
			// duree du mouvement
			...
			case MoveDown:
				...
				break;
			case MoveUpRight:
				arrivalX = getX() + Math.round((float)distance*Math.cos(Math.PI/4));
				arrivalY = getY() + Math.round((float)distance*Math.sin(Math.PI/4));
				break;
			case MoveUpLeft:
				arrivalX = getX() - Math.round((float)distance*Math.cos(Math.PI/4));
				arrivalY = getY() + Math.round((float)distance*Math.sin(Math.PI/4));
